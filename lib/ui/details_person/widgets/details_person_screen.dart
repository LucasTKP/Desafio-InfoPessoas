...
class _DetailsPersonScreenState extends State<DetailsPersonScreen> {
  late MaterialBannerService materialBarService;
  late DetailsPersonViewmodel viewmodel;

  @override
  void initState() {
    super.initState();
    materialBarService = getIt<MaterialBannerService>();
  ...
  void toogleFavorite(PersonModel person) async {
    try {
      await viewmodel.toggleFavorite(person);
      if (mounted) {
        materialBarService.showBanner(
          context: context,
          message: viewmodel.isFavorite ? 'Adicionado aos favoritos.' : 'Removido dos favoritos.',
          color: viewmodel.isFavorite ? Colors.green : Colors.orange,
        );
      }
    } catch (e) {
      if (mounted) {
